...
Two strategic hooks provide research context during design:

| Hook | Trigger | Agents | Mode | Timeout |
|------|---------|--------|------|---------|
| **research-start** | Phase 1 (DISCOVER) | Locator, Pattern, CODEMAPS, Architecture | ALL | 10s |
| **research-verify** | Phase 3→4 (DEVELOP→VERIFY) | Analyzer, Pattern, Impact, Web | FULL only | 15s |

### research-start (Phase 1)

Spawns **before** DISCOVER to ground the session:
1. Extract topic from user's initial message
2. Spawn parallel agents: Locator + Pattern + CODEMAPS loader
3. Display research context box with related files and patterns
4. Continue to DISCOVER with context loaded

### research-verify (Phase 3→4)

Spawns at end of DEVELOP to validate design:
1. Spawn 4 agents: Analyzer + Pattern + Impact + Web
2. Calculate confidence (HIGH/MEDIUM/LOW)
3. Flag conflicts or stale assumptions
4. On LOW confidence: prompt for revision before Phase 4

> **Cross-skill reference:** Load the [conductor](../conductor/SKILL.md) skill for implementation research hooks.

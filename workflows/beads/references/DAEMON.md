...
With sync-branch configured, daemon commits to a dedicated branch using an internal worktree, so your current branch is never affected. See [GIT_INTEGRATION.md](GIT_INTEGRATION.md) for details.
...
| `BEADS_DAEMON_MODE` | `poll`, `events` | `events` | Sync mode (polling vs events) |

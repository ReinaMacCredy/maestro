...
| vNext Authority and continuity | `tests/vnext_authority_{capacity_transition,continuity_totality,contracts,evaluator,literals,publication}.rs`, `tests/vnext_stage0_stage2_literal_proof.rs`, and module-local `authority::{facade,post_cut,publication}::tests` | Run all Store suites and `tests/architecture_imports.rs` when Authority schemas, contexts, bases, Principal/Binding/Session/Grant validation, trusted time, revocation, Mandates, governed capacity, continuity, Receipt/Result carriers, semantic convergence, or same-Store publication changes. Re-run the Stage 0 Effect Home builder and Stage 2 Python builder/check, semantic mutant validator, and independent Ruby verifier for literal changes. Include unknown-tag, stale basis, Store-loaded fact substitution, owner-term totality, same-key replay/changed-meaning, different-key convergence, concurrent publication, exact post-cut root closure, no-refill, and inactive-restore proof. |

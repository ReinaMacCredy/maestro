...
| vNext canonical Store foundation | `tests/vnext_store_contracts.rs`, `tests/vnext_canonical_store.rs`, `tests/vnext_store_safety.rs`, `tests/vnext_store_full_export.rs`, plus module-local `secure_fs`, snapshot-bound, and sealed-backup tests | Run all Store tests when identity, schema, verified database/WAL bindings, deferred read-snapshot admission, precommit/postcommit phase classification, Generation/Head CAS, object publication, reachability, retention/GC, backup receipt/publication recovery, sealed export, restore candidate, compatibility, currentness, or root-path safety changes. Include hostile exact-schema reopen, concurrent WAL writer, precommit rollback, committed-receipt recovery, already-public missing-closure reconstruction, and second-backup import → activate → reseal with every prior closure retained. Broaden to Stage 0 identity/root tests when a domain separator or frozen compatibility commitment changes. |

...
| vNext external fanout history and canonical namespace | `python3 -m unittest tools.vnext_contracts.fanout.test_validate`, `python3 -m unittest tools.vnext_contracts.stage12.test_validate`, `cargo test --test architecture_imports`, `cargo test --test vnext_stage12_contracts` | Run the fanout suite for every historical manifest, stage-owner, comparison-chain, Git-object-store, or external-validator change. Run the Stage 12 suite and both Rust architecture/contract suites whenever a canonical facade, visibility, import edge, exact 210-entry promotion binding, ten-way collision closure, namespace-zero rule, or legacy-pruning blocker changes. |
...
For the historical pre-Stage-12 vNext fanout seam, also run the external fanout Python

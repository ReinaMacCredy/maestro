...
| vNext deterministic identity and candidate Contract | `tests/vnext_manifest_identity.rs`, `tests/vnext_decision_closure.rs`, `tests/vnext_candidate_root.rs`, `tests/vnext_stage0_proof_matrix.rs`, `tests/vnext_submission_claim_set.rs` | All vNext literal, Resource, catalog, and architecture-import tests when a domain separator, canonical shape, materialization, root-binding, finalization, or CBOR rule changes. |
| vNext Stage 0 literals and Resources | `tests/vnext_catalog_literals.rs`, `tests/vnext_public_contract_literals.rs`, `tests/vnext_public_identity.rs`, `tests/vnext_effect_home_literals.rs`, `tests/vnext_dispatch_cutover_literals.rs`, `tests/vnext_resource_census.rs`, `tests/vnext_resource_release.rs` | Re-run the owning Python/Ruby builders, semantic validators, mutant suites, proof-matrix check, candidate-root check, and Resource feedback check before broad Rust verification. |
| vNext canonical Store foundation | `tests/vnext_store_contracts.rs` plus module-local `persistence::tests::{canonical_store,store_safety,store_full_export,atomic_publication}`, `secure_fs`, snapshot-bound, and sealed-backup tests | Run all Store tests when identity, schema, verified database/WAL bindings, deferred read-snapshot admission, precommit/postcommit phase classification, Generation/Head CAS, object publication, reachability, retention/GC, backup receipt/publication recovery, sealed export, restore candidate, compatibility, currentness, idempotency, Store token allocation, or root-path safety changes. Include hostile exact-schema reopen, concurrent WAL writer, precommit rollback, committed-receipt recovery, already-public missing-closure reconstruction, and second-backup import → activate → reseal with every prior closure retained. Broaden to Stage 0 identity/root tests when a domain separator or frozen compatibility commitment changes. |
| vNext Authority and continuity | `tests/vnext_authority_{capacity_transition,continuity_totality,contracts,evaluator,literals,publication}.rs`, `tests/vnext_stage0_stage2_literal_proof.rs`, `tests/vnext_stage5_{contracts,evidence_gates}.rs`, `tests/architecture_imports.rs`, and module-local `authority::{action_basis,facade,post_cut,publication}::tests`, `integration::trusted_host_diagnostic::tests` | Run all Store suites when Authority schemas, contexts, bases, Principal/Binding/Session/Grant validation, trusted time, revocation, Mandates, governed capacity, continuity, Receipt/Result carriers, semantic convergence, same-Store publication, or the protected diagnostic join changes. Re-run the Stage 0 Effect Home builder and Stage 2 Python builder/check, semantic mutant validator, and independent Ruby verifier for literal changes. For the Stage-5 diagnostic seam include owner-local positive and sibling-negative architecture probes for the owner-subtree-sealed Integration and Persistence ports plus the concrete Authority-owned Stage-8 assembler seed; `cfg(test)`-only concrete implementations; the exact closed admission/attempt/generation, reason/carrier/freshness/remediation, witness/currentness input; fixed-capacity Authority re-encoding and exact byte validation with no generic builder, adapter digest, or unbounded byte return; release only after host and Persistence final rechecks; a lifetime-bound Store-currentness lease with consuming final recheck; Active Store before same-view invocation/challenge issuance; independent field-by-field exact-one host-to-Authority matching; one Authority-recomputed attestation commitment used as the witness carrier and revalidated by the live connection; one-shot challenge/attestation consumption; host disconnect/reconnect/reauth/revocation/currentness turnover; restore/incarnation mismatch; heterogeneous single-Authority-root selection; duplicate/stale/foreign refusal; non-oracular subject errors; non-escaping guard/witness; zero writes; and an exact `test_adapter_only` claim that rejects additive or nested production claims. Tags 94–145 must remain typed owner-unavailable and unable to enter any Stage-5 carrier, capacity, debit, or mutation path. |
| vNext external fanout boundary and temporary facades | `python3 -m unittest tools.vnext_contracts.fanout.test_validate`, `cargo test --test architecture_imports`, `cargo test --test vnext_stage5_contracts` | Run the Python suite for every manifest, stage-owner, fanout-base, comparison-chain, candidate-diff, Git-object-store, path-normalization, or external-validator change. It creates self-contained temporary Git repositories and drives the CLI against valid candidates plus missing/substituted/wrong-mode canonical design/Decision inputs; hidden certified-to-fanout and fanout-to-comparison changes; inherited-seed absence, baseline drift, overlap, non-owner/status/mode/path expansion; and graft, alternates, shallow, promisor, replacement-ref, loose-object substitution, and filesystem-alias attacks. Run both Rust suites whenever a temporary facade, visibility, import edge, certified tuple, proof pointer, release path/bytes/mode, payload, plan identity, or snapshot identity changes. |
| vNext Work, Step, Design, Decision, Evidence Claim, and Repository publication | `tests/vnext_{work_identity,work_lifecycle,work_relations,step_graph,step_amendment,step_amendment_application,contract_step_publication,design_revisions,decision_kernel,decision_materialization_plan,evidence_claims,submission_claim_set,stage3_contracts}.rs` plus module-local `repository::tests` | Run all Stage 0 identity/root, Stage 1 Store, and Stage 2 Authority publication suites when lifecycle, Contract-root binding, Step generation Binding, Contract-amendment disposition, Claim-set schema, Decision resolution/materialization, or Repository mutation changes. Re-run the Stage 3 Python builder/check, semantic mutant validator, and independent Ruby verifier for literal changes. Include exact frozen Action descriptors, fail-closed absence of Stage 4 Step Submission and Stage 5 satisfaction-carry handlers and repository-private carrier substitutes, complete initial Step DAG initialization, total Contract-amendment disposition and obligation conservation, full Grant ancestry, stale/hostile Authority, same-key replay/changed-meaning, concurrent loser-no-write, crash rollback with no orphans, candidate-only materialization, and both equal-root Contract-amendment and receipt-proven equivalent-root materialization read-only no-ops with exact Store basis plus no discarded lifecycle/Step/materialization input and no authority/capacity/idempotency artifacts. |
...
For the pre-Stage-12 vNext fanout seam, also run the external fanout Python
suite. Its valid path must authenticate the exact certified tuple and both
hidden history ranges; the three canonical tracked inputs at their deterministic
paths, lengths, SHA-256s, and `100644` modes; and every inherited seed unchanged
from certified Stage 5. Negative paths must reject any missing or substituted
canonical input, undeclared shared or frozen base change, Stage-5 release-object
byte change, inherited-seed absence/drift/overlap/non-owner/status/mode change,
cross-stage comparison checkpoint, deletion, Git graft, alternates/shared object
storage, shallow/promisor/lazy storage, replacement-ref or loose-object
substitution, unsafe or filesystem-aliased tree path, and candidate that is not
the sole direct child of its comparison commit.

...
The plan record in `.maestro/missions/plans.jsonl` carries `title`, `slug`, `spec_path`, and `state` — it does not carry narrative. For non-trivial plans, persist a markdown sidecar so future sessions and reviewers have the rationale:

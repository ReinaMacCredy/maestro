...
- Root `AGENTS.md` is generated by Step 4 (init-deep workflow): a hierarchical PROJECT KNOWLEDGE BASE with the Maestro pointer absorbed inside, still wrapped by `<!-- maestro-setup:start -->` / `<!-- maestro-setup:end -->` markers.
...
### 4. Generate Hierarchical AGENTS.md (init-deep)

...
- **Interface adapters**: `mcp`, `shell`, and `tui` expose alternate operator
  interfaces over the same local substrate.
...
- `shell`: owns shell integration output. It should not own install or Harness
  mutation policy.
...
Current state: several human-facing templates are embedded directly in Rust
source files.

| Content | Current location | Current role |
| --- | --- | --- |
| Harness protocol Markdown | `src/harness/templates.rs` | `HARNESS_MD` raw string installed by `maestro init`. |
| Default Harness YAML and backlog YAML | `src/harness/templates.rs`, `src/harness/schema.rs` | Structured defaults serialized from Rust structs. |
| Bundled skills | `src/skills/bundled.rs` | `SKILL.md` contents embedded as raw strings in the binary. |
...
| Shell init snippets | `src/shell/mod.rs` | Bash/zsh/fish shell snippets returned by `maestro shell-init`. |
| Decision Markdown template | `src/decisions/template.rs` | Small generated decision file body. |
...
- Harness owns `resources/harness/HARNESS.md` and structured Harness defaults.
  ...
  behavior.
- Skills owns `resources/skills/bundled/*/SKILL.md` and extraction behavior.
...
`resources/skills/bundled/<skill-name>/...`, and a drift check should fail when

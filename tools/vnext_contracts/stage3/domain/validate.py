#!/usr/bin/env python3
"""Independently reconstruct and validate the Stage 3 domain-kernel contract."""
...
def load_object(relative: str) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise SystemExit(f"predecessor artifact must contain one object: {relative}")
    return value
...
def predecessor_chain_binding() -> dict[str, object]:
    ...
    if (
        ...
        raise SystemExit("Stage 0 proof receipt is not a final inactive-candidate certification")
    ...
    if stage0_finalization.get("encoder_receipt_sha256") != stage0_encoder_sha256:
        raise SystemExit("Stage 0 finalization receipt does not bind the exact encoder receipt")
    ...
    if not isinstance(stage0_semantic_root, str) or not stage0_semantic_root.startswith("sha256:"):
        raise SystemExit("Stage 0 finalization receipt has no semantic root")
    ...
    if (
        ...
        raise SystemExit("Stage 2 manifest has no exact Stage 0 tree and Stage 2 semantic roots")
    for relative in STAGE2_PROOF_RECEIPTS:
        receipt = load_object(relative)
        if receipt.get("root_id") != stage2_root_id:
            raise SystemExit(f"Stage 2 proof receipt does not bind the exact root: {relative}")
...
def validate_source_closure() -> None:
    ...
    for relative in [
        ...
        if relative not in SOURCE_PATHS:
            raise SystemExit(f"Stage 3 transitive semantic dependency is undeclared: {relative}")


def canonical_value(validate_predecessors: bool = True) -> list[object]:
...
def validate_predecessor_chain() -> None:
    ...
    for command in commands:
        ...
        if result.returncode != 0:
            ...
            raise SystemExit(
                f"Stage 3 predecessor validation failed: {' '.join(command)}: {detail}"
            )
...
def validate_action_catalog() -> None:
    ...
    if (
        ...
        raise SystemExit("the frozen ActionSpec manifest or grammar identity drifted")
    ...
    if (
        ...
        raise SystemExit("the frozen ActionSpec manifest envelope is not self-authenticating")
    ...
    for _, tag, name, descriptor_id, owner_tag, owner_id, local_tag in ACTION_CATALOG[4]:
        ...
        if setup_row is None or generated_row is None or manifest_row is None:
            raise SystemExit(f"missing frozen ActionSpec row {tag}:{name}")
        expected = [name, descriptor_id, owner_tag, owner_id, local_tag]
        actual_setup = [setup_row["name"], setup_row["descriptor_id"], setup_row["primary_owner_tag"], setup_row["primary_owner_descriptor_id"], setup_row["family_local_tag"]]
        ...
        if (
            actual_setup != expected
            or actual_generated != expected
            ...
            raise SystemExit(f"frozen ActionSpec row drifted for {tag}:{name}")
...
def validate_store_schemas() -> None:
    for expected_ordinal, (ordinal, _, domain, schema_id) in enumerate(STORE_SCHEMAS[1], start=1):
        ...
        if ordinal != expected_ordinal or schema_id != expected:
            raise SystemExit(f"Repository Store schema identity drifted for {domain}")


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--root", type=Path, default=WORKSPACE / "contracts/vnext/stage3/domain")
    ...
    if args.catalog_only:
        ...
        return 0
    manifest = json.loads((args.root / "domain-kernel.v1.json").read_text(encoding="ascii"))
    if set(manifest) != {"canonical_value", "identity", "publication_state", "schema_version"}:
        raise SystemExit("Stage 3 manifest has an unknown or missing field")
    expected = canonical_value(validate_predecessors=not args.artifact_only)
    if manifest["canonical_value"] != expected:
        raise SystemExit("Stage 3 canonical projection differs from independently reconstructed semantics")
    encoded = cbor_py.encode(expected)
    if (args.root / "domain-kernel.v1.cbor").read_bytes() != encoded:
        raise SystemExit("Stage 3 CBOR bytes differ from the canonical projection")
    identity = f"sha256:{digest(encoded)}"
    if manifest["identity"] != identity:
        raise SystemExit("Stage 3 identity differs from canonical CBOR sha256")
    if manifest["schema_version"] != DOMAIN or manifest["publication_state"] != PUBLICATION_STATE:
        raise SystemExit("Stage 3 schema or publication state drifted")
    ...
    receipt = {
        ...
        "schema_version": "maestro.vnext.stage3.domain-kernel.semantic-validation-receipt.v1",
        "validation_mode": "full_chain",
        "validator": "independent-python-reconstruction",
    }
    (args.root / "semantic-validation-receipt.v1.json").write_text(

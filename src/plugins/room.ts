...
4. Resolve the started agent's Maestro session before opening its dispatch, without spending a turn: the SessionStart hook has already registered it. Read the pane's process id with \`herdr pane process-info --pane <pane-id>\` and take the session whose pid matches in \`maestro status --live\` (Claude and Codex alike). Herdr's \`agent_session.value\` is a fallback that can drift between turns; when the lane already holds work, the holder shown by \`maestro status --live\` is the authority. Never treat the pane id as session identity. Never send a warm-up prompt just to learn the id.

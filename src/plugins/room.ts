...
5. Start the assigned role with \`herdr agent start peer-<dispatch id> --kind <kind> --pane <pane-id>\`. Pass the chosen model from the Model table in \`maestro recipe show slp\` to the harness: use \`-- --model <name>\` for Claude or Codex's \`--model <name>\` flag (verified with \`codex --help\`). Inspect the agent with \`herdr agent list\`; this applies to \`scout\`, \`decision\`, \`delivery\`, \`challenge\`, and \`shadow\`. Read the stored contract with \`maestro dispatch show <dispatch-id>\` and its work context with \`maestro dispatch list <work-id>\`, then send the exact stored contract with \`herdr agent prompt peer-<dispatch id> "<exact stored contract>"\`. One lane per \`herdr agent prompt\` call, always naming \`peer-<dispatch id>\`, never a shell loop over lanes: a mis-passed argument returns success without delivering. Every later message between roles starts with \`[from <role>]\` and the record id it is about (\`maestro recipe show slp\`, Talking across roles).

...
5. Use the Room-ledger lifecycle for every team intervention:
   ...
   - Emergency override is not routine authority. When \`supervisor-<team>\` is absent or unreachable, the Room may use \`--requested-by supervisor --override-reason "<why>" --override-evidence "<fresh proof>"\`; TeamRuntime rejects it while the team Supervisor is reachable. Explicit owner intervention instead uses \`--requested-by owner --owner-intervention\` with the same override reason and evidence. These authority flags are separate from force-stop \`--reason\` and \`--evidence\`.

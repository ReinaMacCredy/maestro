...
6. A Lead closing work that carries relayed room intent (a room decision id in its title, acceptance, or notes) reports it: \`herdr agent prompt supervisor "[from lead][done w<id> re d<room-id>] <candidate commit; one line on any deviation>"\`. This is one prompt per closed card, after \`maestro work done\`, never before. \`maestro brief\` prints attention findings only, so without this the room cannot see a closure at all; the room still never polls the Lead.

...
4. Resolve the started agent's Maestro session before opening its dispatch. If Herdr supplies \`agent_session.value\` for the agent (Codex), use it. When it does not (Claude), ask the started lane to run \`maestro status --live\` and report the line marked \`(this session)\`. Herdr's \`agent_session.value\` can drift between turns; when the lane already holds work, the holder shown by \`maestro status --live\` is the authority. Never treat the pane id as session identity.

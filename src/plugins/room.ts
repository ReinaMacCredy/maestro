...
4. Resolve the started agent's Maestro session before opening its dispatch. If Herdr supplies \`agent_session.value\` for the agent (Codex), use it. When it does not (Claude), ask the started lane to run \`maestro status --live\` and report the line marked \`(this session)\`. Never treat the pane id as session identity.

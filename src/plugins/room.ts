...
4. Resolve the started agent's Maestro session without spending a turn. Claude fires SessionStart at startup: read the pane's process id with \`herdr pane process-info --pane <pane-id>\` and take the session whose pid matches in \`maestro status --live\`, then open the dispatch with \`--target-session\`. Codex runs SessionStart on its first turn, so open its dispatch pane-bound without \`--target-session\`, send the real brief as the first prompt, and once the lane has accepted (step 8) read the dispatch again (step 6) and verify that \`claimed by\` or \`held by\` equals the pane's session (the \`herdr agent list\` value and the pid in \`maestro status --live\`). When \`claimed by\` matches, the Lead runs \`maestro dispatch confirm <dispatch-id> --session <session-id>\`. On a mismatch, the Lead runs \`maestro dispatch cancel <dispatch-id> --reason wrong-holder\` and opens a new dispatch. When the lane already holds work, the holder shown by \`maestro status --live\` is the authority. Never treat the pane id as session identity. Never send a warm-up prompt just to learn the id.
...
11. After reviewing the handback and closing or re-dispatching the work, close the lane with \`herdr pane close <pane-id>\`, then \`herdr tab close <tab-id>\` once the lanes tab is empty. The pane stays only when the same lane takes the next dispatch. A \`[from lead]\` prompt never changes an assignment: a new objective or a new stop point is a new dispatch (step 5) on the same work item and the same pane, naming the predecessor handback in the objective. Transcripts persist on disk, so closing loses no evidence. A finding returned in a handback is closed by that handback; it becomes a card only when it is the next thing the Lead will actually do.

...
| the SLP Workspace Pack | `src/plugins/resources/SLP.md`, seeded as `~/maestro/SLP.md` only when absent and snapshotted per generation | Hub Supervisor and every SLP role through its scoped section | the owner edits the Hub copy; install and update preserve it, and only the next team generation receives new bytes |
...
Managed Hub files are never hand-edited. A lesson whose target is the SLP
Workspace Pack or a Hub template edits that file's runtime source; the live Hub
picks it up on the next `maestro update`, and a running team stays pinned until
its next generation. Editing the managed Hub copy directly makes the change
disappear at that update.

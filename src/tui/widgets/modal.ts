...
export function layoutModal(parent: Rect, opts: ModalOptions): ModalLayout {
  ...
  const contentHeight = Math.max(
    rows.reduce((height, row, index) => {
      const sectionHeight = opts.mode !== "palette" && !compactRows && row.section && row.section !== rows[index - 1]?.section ? 1 : 0;
      return height + sectionHeight + getRowHeight(row, compactRows, opts.mode);
    }, 0),
    emptyContentHeight,
  );
  ...
  const maxLineLength = Math.max(
    ...
    ...rows.flatMap((row) => [
      opts.mode === "palette"
        ? getPaletteRowLength(row)
        : (row.section?.length ?? 0) + 2,
      opts.mode === "palette"
        ? 0
        : row.label.length + (row.hint?.length ?? 0) + 4,
      opts.mode === "palette" ? 0 : (row.detail?.length ?? 0),
    ]),
  );

  const preferredWidth = opts.mode === "palette" ? 94 : 50;
  ...
  if (opts.mode !== "info") {
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      if (opts.mode !== "palette" && !compactRows && row.section && row.section !== previous?.section) {
        currentY += 1;
      }
      const remainingHeight = contentRect.y + contentRect.height - currentY;
      if (remainingHeight <= 0) break;
      const height = Math.min(getRowHeight(row, compactRows, opts.mode), remainingHeight);
      if (row.selectable) {
...
export function renderModal(buf: Buffer, parent: Rect, opts: ModalOptions): ModalLayout {
  ...
  if (opts.mode === "palette") {
    renderQueryRow(buf, layout, opts.query);
  ...
  if (opts.mode === "palette" && rows.length === 0) {
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      const rowRect = opts.mode === "info"
        ? { x: layout.x + 1, y: rowY, width: layout.width - 2, height: getRowHeight(row, compactRows, opts.mode) }
        : layout.itemRects[layout.itemRowIndexes.indexOf(index)] ?? {
          x: layout.x + 1,
          y: rowY,
          width: layout.width - 2,
          height: getRowHeight(row, compactRows, opts.mode),
        };
...
function renderRow(
  ...
  if (mode === "palette") {
    renderPaletteRow(buf, rect, row, isSelected);
    return;
  }
...
function getRowHeight(row: NormalizedModalRow, compact = false, mode?: ModalOptions["mode"]): number {
  if (mode === "palette" || compact) return 1;
...
function renderPaletteRow(
  ...
  const bg = isSelected ? PALETTE.yellow : PALETTE.overlaySurfaceBg;
  const fg = isSelected ? PALETTE.headerBg : PALETTE.brightWhite;

  buf.fillRect(rect, " ", { bg, fg: PALETTE.default, bold: false, dim: false });

  if (row.section) {
    buf.writeText(rect.y, sectionX, truncate(row.section.toLowerCase(), sectionWidth), {
      fg: isSelected ? PALETTE.headerBg : PALETTE.gray,
      bg,
      bold: isSelected,
      dim: false,
    });
  ...
  buf.writeText(rect.y, labelX, truncate(row.label.toLowerCase(), labelMax), {
    fg,
    bg,
    bold: true,
    dim: false,
  ...
  if (row.hint && hintX && hintX > labelX + 2) {
    buf.writeText(rect.y, hintX, row.hint, {
      fg: isSelected ? PALETTE.headerBg : PALETTE.blue,
      bg,
      bold: true,
      dim: false,
    });

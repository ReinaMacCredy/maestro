...
// Step 5: migrate-tasks — translate v1 task rows into tasks.v2.jsonl.
export async function runMigrateTasks(
  ...
  if (!(await fileExists(source))) {
    return {
      id: "migrate-tasks",
      label: "Translate v1 tasks into tasks.v2.jsonl",
      status: "skipped",
      detail: "no v1 tasks.jsonl",
    };
  }
  const dest = join(deps.repoRoot, ".maestro/tasks/tasks.v2.jsonl");
  ...
  for (const line of lines) {
    ...
    await appendFile(dest, `${JSON.stringify(v2)}\n`, "utf8");
    migrated++;
  }
  return {
    id: "migrate-tasks",
...
// Step 6: migrate-plans — translate v1 mission directories into plans.v2.jsonl.
export async function runMigratePlans(
  ...
  if (!(await dirExists(sourceDir))) {
    return {
      id: "migrate-plans",
      label: "Translate v1 missions/plans into plans.v2.jsonl",
      status: "skipped",
      detail: "no .maestro/missions/",
    };
  }
  await mkdir(join(deps.repoRoot, ".maestro/plans"), { recursive: true });
  const dest = join(deps.repoRoot, ".maestro/plans/plans.v2.jsonl");

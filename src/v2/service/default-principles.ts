...
Passive harness is the load-bearing v2 invariant: a single repo-tracked state directory can be safely shared across agents and operators only when nothing is mutating the world while it is being read.

## Scan Command

! rg -n "setInterval|setTimeout|child_process\\.fork|spawn.*detached|new Worker\\(" --glob 'src/v2/**' --glob '!**/*.test.ts'

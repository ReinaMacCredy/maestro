...
export async function runArchitectureLints(
  ...
  const scopes = rules.lint_scope.length === 0 ? [DEFAULT_SCOPE] : rules.lint_scope;
...
export function resolveImportTargetLayer(
  ...
  if (importStr.startsWith("@/v2/")) {
    const after = importStr.slice("@/v2/".length);
  ...
  for (const pattern of rules.passive_harness.forbidden_patterns) {
    ...
    while ((match = re.exec(text)) !== null) {
      violations.push({
        ...
        file: relFile,
        line: lineOf(text, match.index),
        message: `forbidden pattern ${JSON.stringify(pattern)} found; v2 must remain passive per docs/architecture.yaml`,
      });
    }
  }
  return violations;

...
interface TaskProofDeps {
  readonly getServices: () => Pick<Services, "taskStore" | "evidenceStore" | "specStore">;
}

export function registerTaskProofCommand(
  taskCmd: Command,
  program: Command,
  deps: TaskProofDeps = { getServices },
): void {
  taskCmd
    .command("proof")
    ...
    .action(async (opts): Promise<void> => {
      ...
      // 4. Build proof map
      const proofMap = buildProofMap({ taskId, spec, evidenceRows });
...
function printTextProofMap(proofMap: ProofMap): void {
  if (proofMap.entries.length === 0) {
    console.log(`Proof map for task ${proofMap.taskId}: no spec linked`);
    return;
  }
  ...
  console.log(
    `Proof map for task ${proofMap.taskId} (${coveredCount}/${total} covered):`,
  );

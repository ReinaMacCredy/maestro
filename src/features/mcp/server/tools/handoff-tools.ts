...
export function registerHandoffTools(server: McpServer, deps: RegisterDeps): void {
  server.registerTool(
    "maestro_handoff_list",
    {
      ...
      description:
        "List handoff packets visible from the current maestro project, newest first. Filters: openOnly, displayState (open|consumed|completed|failed), taskId, agent. Paginated (default limit 20). view='summary' (default) returns name+id+status+task+agent+createdAt; view='full' returns the full record. Read-only.",
    ...
    async (args): Promise<CallToolResult> => {
      try {
        ...
        const projected = args.view === "full"
          ? page
          : { ...page, items: page.items.map(summarizeHandoff) };

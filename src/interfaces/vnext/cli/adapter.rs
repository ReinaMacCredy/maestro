...
#![allow(
    dead_code,
    reason = "Stage 6 is candidate-only until the separately owned root CLI is activated"
)]
...
use crate::domain::vnext::integration::public_literals::{
    OperationResultV1, OperationSemanticOutcomeV1,
};
use crate::domain::vnext::projection::{ProjectionReadPortV1, read_packet};
use crate::domain::vnext::transport::{
    decode_operation_request, decode_packet_read_request, encode_operation_result,
    encode_packet_read_envelope,
};
use crate::operations::vnext::action::{
...
fn render_prepared(prepared: PreparedOperationV1, json_output: bool) -> Stage6CliOutputV1 {
    let spec_ref = match &prepared.operation_spec {
        crate::domain::vnext::integration::public_literals::OperationSpecRefV1::Action(spec) => {
            &spec.exact_action_spec_ref
        }
        crate::domain::vnext::integration::public_literals::OperationSpecRefV1::Ceremony(spec) => {
    ...
    if json_output {
        let value = json!({
            ...
            "ceremony_context": prepared.ceremony_context.map(|value| match value {
                crate::domain::vnext::capability::generated_catalog::CeremonyContextKindV1::NoStore => "NoStore",
                crate::domain::vnext::capability::generated_catalog::CeremonyContextKindV1::PreStore => "PreStore",
            }),
        });
        match serde_json::to_string(&value) {
            Ok(value) => Stage6CliOutputV1::success(format!("{value}\n")),
...
mod tests {
    use super::*;
    use crate::domain::vnext::integration::public_literals::{
        McpPacketReadRequestV1, OperationRequestV1,
    };
    use crate::domain::vnext::projection::{ProjectionErrorV1, ProjectionReadStateV1};
    use crate::operations::vnext::action::{
    ...
    impl OperationResultReadPortV1 for EmptyResultReadPort {
        fn read_result(
            ...
            Option<(
                OperationRequestV1,
                crate::domain::vnext::integration::public_literals::OperationResultV1,
            )>,

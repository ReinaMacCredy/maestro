...
impl<'cut> LegacyRemovalGuardV2<'cut> {
    ...
    pub(super) fn consume_with_linearization<T, E>(
        self,
        closure: &Stage12ConsumerReaderHoldClosureV2,
...
#[derive(Debug, Error, Eq, PartialEq)]
pub(in crate::domain) enum LegacyRemovalGuardAdmissionErrorV3 {
    #[error("the complete V4 owner-issued legacy removal facts do not describe one exact cut")]
    OwnerFactsMismatch,
    #[error("the live Authority state is absent, inactive, or not an Installation authority")]
    InvalidAuthorityState,
    #[error(
        "the live Authority currentness or revocation state changed before V4 pruning linearization"
    )]
    AuthorityCurrentnessDrift,
    #[error("Authority could not issue a unique process-local V4 pruning invocation")]
    InvocationUnavailable,
    #[error("V4 legacy removal requires Authority-bracketed expected-old linearization")]
    LinearizationRequired,
}

pub(super) struct LegacyRemovalGuardBindingV3 {
    _realm: [u8; 32],
    _release: [u8; 32],
    _store_generation: [u8; 32],
    _store_head: [u8; 32],
    _invocation: LegacyRemovalInvocationBindingV3,
    _source_case_manifest: [u8; 32],
    _sighting_manifest: [u8; 32],
    _classification_manifest: [u8; 32],
    _overlap_manifest: [u8; 32],
    _loss_manifest: [u8; 32],
    _quarantine_manifest: [u8; 32],
    _foundation_closure: [u8; 32],
    _rollback_assessment: [u8; 32],
    _quarantine_epoch: [u8; 32],
    _replacement_activation: [u8; 32],
    _consumer_closure: [u8; 32],
    _reader_closure: [u8; 32],
...
pub(super) struct LegacyRemovalInvocationBindingV3 {
    commitment: [u8; 32],
    process_incarnation: [u8; 32],
    entropy: [u8; 32],
    sequence: u64,
...
impl LegacyRemovalInvocationBindingV3 {
    #[expect(
        clippy::too_many_arguments,
        reason = "the unique V4 pruning invocation binds every current cut identity"
    )]
    pub(super) fn mint(
        process_incarnation: [u8; 32],
        entropy: [u8; 32],
        ...
        Ok(Self {
            commitment: legacy_removal_invocation_commitment_v3(
                &process_incarnation,
                &entropy,
                sequence,
                current_head,
    ...
    #[expect(
        clippy::too_many_arguments,
        reason = "the unique V4 pruning invocation rechecks every current cut identity"
    )]
    fn recheck(
        &self,
        current_head: &[u8; 32],
        ...
        self.process_incarnation != [0; 32]
            ...
            && self.sequence != 0
            && self.commitment
                == legacy_removal_invocation_commitment_v3(
                    &self.process_incarnation,
                    &self.entropy,
                    self.sequence,
                    current_head,
...
#[expect(
    clippy::too_many_arguments,
    reason = "the V3 invocation domain commits every V4-bound cut identity"
)]
fn legacy_removal_invocation_commitment_v3(
    process_incarnation: &[u8; 32],
    entropy: &[u8; 32],
    sequence: u64,
    current_head: &[u8; 32],
    ...
    for field in [
        ...
        sequence_bytes.as_slice(),
        current_head.as_slice(),
        loss_manifest.as_slice(),
        foundation_closure.as_slice(),
        rollback_assessment.as_slice(),
        epoch.as_slice(),
        activation.as_slice(),
        deletion_plan.as_slice(),
    ] {
...
#[derive(Clone, Copy)]
pub(super) struct LegacyRemovalGuardCurrentnessV3 {
    pub(super) realm: [u8; 32],
    pub(super) store_generation: [u8; 32],
    pub(super) store_head: [u8; 32],
    pub(super) authority_snapshot: [u8; 32],
...
#[derive(Clone, Copy)]
pub(super) struct LegacyRemovalConsumerBindingV3 {
    pub(super) consumer_closure: [u8; 32],
    pub(super) release: [u8; 32],
    pub(super) source_case_manifest: [u8; 32],
    pub(super) sighting_manifest: [u8; 32],
    pub(super) classification_manifest: [u8; 32],
    pub(super) overlap_manifest: [u8; 32],
    pub(super) loss_manifest: [u8; 32],
    pub(super) quarantine_manifest: [u8; 32],
    pub(super) foundation_closure: [u8; 32],
    pub(super) rollback_assessment: [u8; 32],
    pub(super) quarantine_epoch: [u8; 32],
    pub(super) replacement_activation: [u8; 32],
    pub(super) reader_closure: [u8; 32],
    pub(super) hold_closure: [u8; 32],
...
impl LegacyRemovalConsumerBindingV3 {
    fn observe(closure: &Stage12ConsumerReaderHoldClosureV3) -> Self {
        Self {
            consumer_closure: *closure.identity().as_bytes(),
            release: *closure.release_id().as_bytes(),
            source_case_manifest: *closure.source_case_manifest_id().as_bytes(),
            sighting_manifest: *closure.sighting_manifest_id().as_bytes(),
            classification_manifest: *closure.classification_manifest_id().as_bytes(),
            overlap_manifest: *closure.overlap_manifest_id().as_bytes(),
            loss_manifest: *closure.loss_manifest_id().as_bytes(),
            quarantine_manifest: *closure.quarantine_manifest_id().as_bytes(),
            foundation_closure: *closure.foundation_closure_id().as_bytes(),
            rollback_assessment: *closure.rollback_assessment_id().as_bytes(),
            quarantine_epoch: *closure.legacy_quarantine_epoch_id().as_bytes(),
            replacement_activation: *closure.replacement_activation_id().as_bytes(),
            reader_closure: *closure.physical_pruning_reader_zero_id().as_bytes(),
            hold_closure: *closure.physical_pruning_hold_zero_id().as_bytes(),
...
impl LegacyRemovalGuardBindingV3 {
    #[expect(
        clippy::too_many_arguments,
        reason = "the V3 removal guard binds every independently owner-issued V4 pruning fact"
    )]
    pub(super) const fn new(
        realm: [u8; 32],
        release: [u8; 32],
        store_generation: [u8; 32],
        store_head: [u8; 32],
        invocation: LegacyRemovalInvocationBindingV3,
        source_case_manifest: [u8; 32],
        sighting_manifest: [u8; 32],
        classification_manifest: [u8; 32],
        overlap_manifest: [u8; 32],
        ...
        Self {
            ...
            _loss_manifest: loss_manifest,
            _quarantine_manifest: quarantine_manifest,
            _foundation_closure: foundation_closure,
            _rollback_assessment: rollback_assessment,
            _quarantine_epoch: quarantine_epoch,
            _replacement_activation: replacement_activation,
            _consumer_closure: consumer_closure,
            _reader_closure: reader_closure,
    ...
    fn recheck(
        &self,
        currentness: LegacyRemovalGuardCurrentnessV3,
        consumer: LegacyRemovalConsumerBindingV3,
    ) -> Result<(), LegacyRemovalGuardAdmissionErrorV3> {
        if !self._invocation.recheck(
            &self._store_head,
            &self._loss_manifest,
        ...
        if [
            self._source_case_manifest,
            self._sighting_manifest,
            self._classification_manifest,
            self._overlap_manifest,
            self._loss_manifest,
        ...
        if self._consumer_closure != consumer.consumer_closure
            || self._release != consumer.release
            || self._source_case_manifest != consumer.source_case_manifest
            || self._sighting_manifest != consumer.sighting_manifest
            || self._classification_manifest != consumer.classification_manifest
            || self._overlap_manifest != consumer.overlap_manifest
            || self._loss_manifest != consumer.loss_manifest
            || self._quarantine_manifest != consumer.quarantine_manifest
            || self._foundation_closure != consumer.foundation_closure
            || self._rollback_assessment != consumer.rollback_assessment
            || self._quarantine_epoch != consumer.quarantine_epoch
            || self._replacement_activation != consumer.replacement_activation
            || self._reader_closure != consumer.reader_closure
            || self._hold_closure != consumer.hold_closure
    ...
    pub(super) fn test_from_currentness(
        ...
        Self::new(
            ...
            LegacyRemovalInvocationBindingV3::mint(
                ...
                1,
                &currentness.store_head,
                &consumer.loss_manifest,
                &consumer.foundation_closure,
                &consumer.rollback_assessment,
                &consumer.quarantine_epoch,
                &consumer.replacement_activation,
                &consumer.deletion_plan,
            )
            ...
            consumer.sighting_manifest,
            consumer.classification_manifest,
            consumer.overlap_manifest,
            consumer.loss_manifest,
            consumer.quarantine_manifest,
            consumer.foundation_closure,
            consumer.rollback_assessment,
            consumer.quarantine_epoch,
            consumer.replacement_activation,
...
pub(in crate::domain) struct LegacyRemovalGuardV3<'cut> {
    _binding: LegacyRemovalGuardBindingV3,
    _store: &'cut mut StoreV1,
    _not_send_or_sync: PhantomData<Rc<()>>,
}

impl<'cut> LegacyRemovalGuardV3<'cut> {
    pub(super) const fn mint(
        binding: LegacyRemovalGuardBindingV3,
        store: &'cut mut StoreV1,
    ) -> Self {
        Self {
            _binding: binding,
    ...
    pub(in crate::domain) fn consume_for(
        ...
        match self.consume_with_linearization(closure, || {
            Err::<std::convert::Infallible, _>(
                LegacyRemovalGuardAdmissionErrorV3::LinearizationRequired,
            )
        })? {
            Ok(never) => match never {},
            Err(error) => Err(error),
    ...
    pub(in crate::domain) fn consume_with_linearization<T, E>(
        ...
        self.consume_bound_with_linearization(
            LegacyRemovalConsumerBindingV3::observe(closure),
            linearize_expected_old,
        )
    }
    ...
    fn consume_bound_with_linearization<T, E>(
        ...
        match store.with_serialized_active_view(|view| {
            let currentness = super::facade::observe_legacy_removal_guard_currentness_v3(view)?;
            binding.recheck(currentness, consumer)?;
            Ok(linearize_expected_old())
        }) {
            Ok(result) => Ok(result),

...
use crate::domain::identity::{SchemaIdV1, StoreGenerationIdV1, StoreObjectIdV1};
use crate::domain::persistence::StoreObjectV1;
...
macro_rules! owner_family_authority {
    (
        ...
        impl $type {
            ...
            pub(in crate::domain::authority) const fn selection(
                self,
            ) -> RepositoryAuthoritySelectionV1 {
                self.0.selection()
            }

            pub(in crate::domain::authority) const fn action(
                self,
            ) -> RepositoryDownstreamActionLeafV1 {
                self.0.action()
            }

            pub(in crate::domain::authority) const fn subject_commitment(self) -> [u8; 32] {
                self.0.subject_commitment()
            }

            pub(in crate::domain::authority) const fn current_semantic_owner_basis_commitment(
                self,
            ) -> [u8; 32] {
                self.0.current_semantic_owner_basis_commitment()
            }

            pub(in crate::domain::authority) const fn exact_payload_commitment(self) -> [u8; 32] {
...
-> Result<SchemaIdV1, crate::domain::identity::IdentityError> {
...
pub(crate) enum RepositoryLeafAuthorityEvaluationErrorV1 {
    ...
    #[error(transparent)]
    StoreObject(#[from] crate::domain::persistence::StoreObjectError),
    #[error(transparent)]
    CanonicalCbor(#[from] CborError),
    #[error(transparent)]
...
mod tests {
    ...
    fn owner_family_authority_names_are_confined_to_authority_and_the_exact_semantic_owner() {
        const OWNER_ALLOWLIST: [(&str, &str); 8] = [
            (
                "CoordinationRepositoryActionAuthorityV1",
                "src/domain/coordination",
            ),
            ("PlanningRepositoryActionAuthorityV1", "src/domain/planning"),
            (
                "PersistenceRepositoryActionAuthorityV1",
            ...
            (
                "DistributionRepositoryActionAuthorityV1",
                "src/domain/distribution",
            ),
            (
                "SearchMaintenanceRepositoryActionAuthorityV1",
                "src/domain/search",
            ),
            ("MemoryRepositoryActionAuthorityV1", "src/domain/memory"),
            ("IntakeRepositoryActionAuthorityV1", "src/domain/intake"),
            ("ResearchRepositoryActionAuthorityV1", "src/domain/research"),
        ];
        ...
        for source_path in sources {
            ...
            for (authority_type, owner_root) in OWNER_ALLOWLIST {
                if identifiers
                    ...
                    && !path_is_within(&relative, "src/domain/authority")

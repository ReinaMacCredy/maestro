...
#[cfg(test)]
use crate::domain::persistence::ProtectedDiagnosticTestCurrentViewProviderV1;
...
pub(super) use crate::domain::persistence::{
...
pub(in crate::domain) use crate::domain::persistence::{
...
#[derive(Debug)]
pub(in crate::domain) enum AuthorityMaterializationPublicationErrorV1<E> {
    Store(crate::domain::persistence::StoreError),
    Prepare(E),
}
...
struct SchedulingPolicyDowngradeMaterializationV1 {
    ...
    human_principal_id: crate::domain::authority::PrincipalIdV1,
    human_binding_id: crate::domain::authority::PrincipalBindingIdV1,
    human_session_id: crate::domain::authority::SessionIdV1,
...
impl MaterializationAtomicPublicationPortV1 for AuthorityMaterializationPublicationViewV1<'_> {
    fn expected_old(&self) -> Option<crate::domain::identity::StoreHeadIdV1> {
    ...
    fn generation_previous(&self) -> Option<crate::domain::identity::StoreGenerationIdV1> {
...
impl MaterializationStoreViewPortV1 for StorePublicationViewV1<'_> {
    fn active_head_id(
        &self,
    ) -> Result<Option<crate::domain::identity::StoreHeadIdV1>, AuthorityMaterializationErrorV1>
    {
        self.active_head()
    ...
    fn active_generation_id(
        &self,
    ) -> Result<crate::domain::identity::StoreGenerationIdV1, AuthorityMaterializationErrorV1> {
...
impl<'tx> AuthorityMaterializationPortV1<'tx> {
    ...
    pub(in crate::domain::authority) fn admit_binding<K: RepositoryActionBindingKindV1>(
        &'tx self,
    ...
    pub(in crate::domain::authority) fn execute_scheduling_downgrade_materialization(
        &'tx self,
        input: RepositoryActionAdmissionInputV1,
...
fn resolve_scheduling_policy_downgrade_mandate(
    ...
    current_head: &crate::domain::persistence::StoreHeadV1,
    current_generation: &StoreGenerationV1,
    active_objects: &[StoreObjectV1],
    owner: &SchedulingPolicyOwnerPublicationV1,
...
impl<'store> AuthorityFacadeV1<'store> {
    ...
    #[expect(
        ...
        reason = "the named pruning admission accepts each independently owner-issued manifest"
    )]
    pub(in crate::domain) fn admit_legacy_physical_pruning<'cut>(
        &'cut mut self,
    ...
    pub(in crate::domain::authority) fn publish_repository_materialization<E>(
        &mut self,
        probe: &StoreIdempotencyProbeV1,
...
fn validate_legacy_physical_pruning_admission_v2(
    ...
    rollback: &Stage12RollbackRehearsalV2,
    deletion_plan: &InstallationLegacyDeletionPlanV2,
) -> Result<LegacyRemovalGuardBindingV2, LegacyRemovalGuardAdmissionErrorV2> {
    if view.role() != StoreRoleV1::Installation {
    ...
    Ok(LegacyRemovalGuardBindingV2::new(
        *current_generation.domain().id().as_bytes(),
        *epoch.release_id().as_bytes(),
        *current_generation.id().as_bytes(),
        *current_head.id().as_bytes(),
...
fn mint_legacy_removal_invocation(
    ...
    let candidate = protected_diagnostic_random_entropy(
        b"maestro.authority.legacy-removal-process-incarnation.v2\0",
    );
    ...
    let sequence = NEXT_INVOCATION
        ...
        .map_err(|_| LegacyRemovalGuardAdmissionErrorV2::InvocationUnavailable)?;
    let entropy = protected_diagnostic_random_entropy(

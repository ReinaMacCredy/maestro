...
fn validate_selected_capability_resolution<'a>(
    ...
    if !is_strictly_ordered_unique(&direct_methods)
        || direct_methods.iter().any(|method| {
            !crate::domain::capability::literals::job_method_is_admitted(selected_job, *method)
        })
        || (!plan.tdd_child_resource_refs.is_empty()
            && (selected_job != InternalJobV1::Execute
                || !direct_methods
                    .contains(&crate::domain::capability::literals::DirectMethodV1::Tdd)))
...
pub enum PublicLiteralError {
    ...
    #[error(transparent)]
    Recipe(#[from] crate::domain::orchestration::literals::RecipeLiteralError),
    #[error(transparent)]
    Capability(#[from] crate::domain::capability::literals::CapabilityLiteralError),
}

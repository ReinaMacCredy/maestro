...
fn load_current_authority(
    ...
    let role = view.role();
    let known_schema_ids = AuthoritySchemaV1::ALL
        .into_iter()
        .map(AuthoritySchemaV1::id)
        .collect::<Result<Vec<_>, _>>()?;
    if active_objects
        .iter()
        .any(|object| !known_schema_ids.contains(&object.schema_id()))
    {
...
fn build_successor_continuity(
    ...
    let class_entries = continuity_class_entries(&current.manifest, &semantic_cut)?;

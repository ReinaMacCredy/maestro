...
use maestro::domain::vnext::identity::{
    ContractRootIdV1, SchemaIdV1, StoreGenerationIdV1, StoreHeadIdV1,
};
...
fn object(
    seed: u64,
    references: Vec<maestro::domain::vnext::identity::StoreObjectIdV1>,
) -> StoreObjectV1 {
    StoreObjectV1::new(
...
fn object_file_path(
    object_id: maestro::domain::vnext::identity::StoreObjectIdV1,
) -> std::path::PathBuf {
...
fn stale_different_key_publication_leaves_no_raw_object_debt() {
    ...
    assert!(matches!(
        error,
        maestro::domain::vnext::persistence::StoreError::HeadCasMismatch
    ));

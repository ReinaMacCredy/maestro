...
#[cfg(test)]
pub(in crate::domain) struct Stage9ActiveStoreFinalitySeedV1 {
    _private: (),
}

#[cfg(test)]
impl Stage9ActiveStoreFinalitySeedV1 {
    pub(in crate::domain) fn test_unavailable() -> Self {
        Self { _private: () }
    }
}
...
pub(in crate::domain) struct Stage9ActiveStoreFinalitySeedV2 {
...
fn validate_owner_publication(
    ...
    let expected_old = publication
        .expected_old()
        .ok_or(DurableInstallationFinalityErrorV2::CurrentnessMismatch)?;
    let successor = crate::domain::persistence::StoreHeadV1::new(
...
fn validate_decision_objects(
    decision: ActiveStoreDecisionTupleV1,
    objects: &[crate::domain::persistence::StoreObjectV1],
) -> Result<(), DurableInstallationFinalityErrorV2> {
...
fn required_object(
    objects: &[crate::domain::persistence::StoreObjectV1],
    expected: [u8; 32],
) -> Result<&crate::domain::persistence::StoreObjectV1, DurableInstallationFinalityErrorV2> {
    objects
        .iter()
...
fn object_set_commitment(objects: &[crate::domain::persistence::StoreObjectV1]) -> [u8; 32] {
...
fn publication_postcondition(
    publication: &AtomicGenerationPublicationV1,
    successor_head: crate::domain::identity::StoreHeadIdV1,
) -> [u8; 32] {
    let mut digest = Sha256::new();
...
mod tests {
    ...
    fn real_publication_fixture(
        ...
        let successor_head = crate::domain::persistence::StoreHeadV1::new(
            publication.generation(),
            publication.generation().ordinal(),
            Some(head.id()),
        )

...
use crate::domain::persistence::CollectionPlanV1;
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(in crate::domain) struct Stage9ActiveConsumerMaterializationV1 {
...
impl Stage9ActiveConsumerMaterializationV1 {
    pub(in crate::domain) fn mint_pre_currentness(
    ...
    pub(in crate::domain) fn mint_protected_retention(
        transaction: &DistributionTransactionV1,
        census: &InstallationCensusV1,
        claims: &InstalledResourceClaimSetV1,
        catalog: &OrdinarySnapshotCatalogV1,
    ...
    pub(in crate::domain) fn mint_physical_pruning(
        transaction: &DistributionTransactionV1,
        census: &InstallationCensusV1,
        claims: &InstalledResourceClaimSetV1,
        catalog: &OrdinarySnapshotCatalogV1,
    ...
    pub(in crate::domain) const fn owner_operation(&self) -> [u8; 32] {
        self.owner_operation
    }

    pub(in crate::domain) const fn owner_stage_tag(&self) -> u8 {
        self.owner_stage_tag
    }

    pub(in crate::domain) const fn installation_id(&self) -> [u8; 32] {
    ...
    pub(in crate::domain) const fn domain(&self) -> [u8; 32] {
        self.domain
    }

    pub(in crate::domain) const fn census_identity(&self) -> [u8; 32] {
        self.census_identity
    }

    pub(in crate::domain) fn census_rows(&self) -> &[[u8; 32]] {
        &self.census_rows
    }

    pub(in crate::domain) const fn release_identity(&self) -> [u8; 32] {
        self.release_identity
    }

    pub(in crate::domain) const fn declared_consumer_root_manifest(&self) -> [u8; 32] {
        self.declared_consumer_root_manifest
    }

    pub(in crate::domain) const fn public_resource_closure(&self) -> [u8; 32] {
        self.public_resource_closure
    }

    pub(in crate::domain) const fn public_bundle_closure(&self) -> [u8; 32] {
        self.public_bundle_closure
    }

    pub(in crate::domain) const fn public_release_closure(&self) -> [u8; 32] {
        self.public_release_closure
    }

    pub(in crate::domain) const fn alias_roots(&self) -> [u8; 32] {
        self.alias_roots
    }

    pub(in crate::domain) const fn manager_roots(&self) -> [u8; 32] {
        self.manager_roots
    }

    pub(in crate::domain) const fn target_roots(&self) -> [u8; 32] {
        self.target_roots
    }

    pub(in crate::domain) const fn claims_catalog_descriptors(&self) -> [u8; 32] {
        self.claims_catalog_descriptors
    }

    pub(in crate::domain) const fn activation_carrier_identity(&self) -> [u8; 32] {
        self.activation_carrier_identity
    }

    pub(in crate::domain) const fn activation_carrier_revision(&self) -> u64 {
        self.activation_carrier_revision
    }

    pub(in crate::domain) const fn activation_attempt_identity(&self) -> [u8; 32] {
        self.activation_attempt_identity
    }

    pub(in crate::domain) const fn activation_destination_seal(&self) -> [u8; 32] {
        self.activation_destination_seal
    }

    pub(in crate::domain) const fn expected_head(&self) -> Option<[u8; 32]> {
        self.expected_head
    }

    pub(in crate::domain) fn required_current_objects(&self) -> &[StoreObjectIdV1] {
        &self.required_current_objects
    }
}

#[derive(Debug, Error)]
pub(in crate::domain) enum InstallationConsumerMaterializationErrorV1 {
    ...
    #[error(transparent)]
    Transaction(#[from] crate::domain::distribution::runtime::DistributionTransactionErrorV1),
    #[error(transparent)]
    Census(#[from] super::InstallationCensusErrorV1),
    #[error(transparent)]
    Records(#[from] crate::domain::distribution::runtime::DistributionRecordErrorV1),
    #[error(transparent)]

...
use super::*;
use crate::domain::authority::test_support::{
...
use crate::domain::contract::materialization::{
    ContractConsequencePlanV1, PlannedContractComponentV1, PlannedContractSlotV1,
};
use crate::domain::contract::provenance::ComponentProvenanceV1;
...
use crate::domain::execution::StepSubmissionExecutionFenceV1;
...
use crate::domain::identity::{DesignRevisionIdV1, DesignSourceBindingIdV1};
use crate::domain::persistence::StoreDomainV1;
...
fn work_completion_requires_and_commits_the_exact_current_satisfied_step_submission_closure() {
    ...
    let current_work = WorkRecordV1::create_draft(WorkRecordWriterV1::Work, work_id)
        ...
        .apply(
            ...
            crate::domain::work::WorkRevisionV1::new(1).expect("test fixture"),
            WorkTransitionV1::PublishInitialContract,
        )
        .expect("test fixture")
        .apply(
            WorkRecordWriterV1::Work,
            crate::domain::work::WorkRevisionV1::new(2).expect("test fixture"),
...
fn evidence_authority_for(
    ...
    request: &crate::domain::evidence::CanonicalEvidenceActionRequestV1,
...
fn work_completion_atomically_persists_claim_gate_and_submission_proof() {
    ...
    let wrong_gate_snapshot = GateSnapshotV1::new(
        ...
        contract_generation.id(),
        contract_generation.root_id(),
        crate::domain::identity::ContractComponentIdV1::from_digest([254; 32]),
        [82; 32],
        [83; 32],
        vec![gate.id()],
        vec![gate.clone()],
...
fn equal_root_materialization_is_store_validated_without_an_action() {
    ...
    assert!(matches!(
        ...
        crate::domain::design::DecisionMaterializationDispositionV1::NoOpEqualRoot
    ));
    assert_eq!(
        store
            .active_head()
...
fn distinct_exactly_equivalent_root_is_receipt_proven_and_zero_write() {
    ...
    assert!(matches!(
        materialization.disposition(),
        crate::domain::design::DecisionMaterializationDispositionV1::NoOpEquivalent { .. }
    ));
...
fn resolved_materialization_decision(
    repository_id: crate::domain::identity::StoreDomainIdV1,
    work_id: WorkIdV1,

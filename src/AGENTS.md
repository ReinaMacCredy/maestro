...
| Product-spec authoring (spec new/validate) | `src/v2/runtime/spec.command.ts` | v2 spec lifecycle; product-spec files live at `.maestro/specs/<slug>.md` |
...
| Trust Verifier (task verify) | `features/verdict/verify/` | `runTrustVerifier` in `trust-verifier.ts`; 7 checks under `checks/`; `TrustFinding`, `TrustVerifierResult` in `@/v2/types/trust.ts` |
| Arch-rules lint (9 rules) | `shared/lib/arch-rules.ts` | `checkArchitectureRules`; consumed by `gc/slop-cleanup` and `ci/run-ci-verify` |
| v2 task lifecycle | `src/v2/runtime/task.command.ts`, `src/v2/service/` | `task claim`, `task verify`, `task block`, `task ship`, `task abandon`; state machine at `src/v2/types/task-state.ts` |
...
| Cost-budget run-state | `features/plan/` | `RunState`, `RunStateStorePort`, plan-check + cost-budget survived Phase 5 in `src/features/plan/`. Run-state persisted under `.maestro/runs/<task-id>/state.json` (gitignored). |

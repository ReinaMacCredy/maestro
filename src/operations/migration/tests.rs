...
use crate::domain::execution::{ActiveStoreEffectSnapshotV1, ActiveStoreEffectWithdrawalOutcomeV1};
...
const ASSOCIATION_RUNTIME_SOURCE: &str =
    include_str!("../../domain/migration/runtime/association.rs");
const ROLLBACK_RUNTIME_SOURCE: &str = include_str!("../../domain/migration/runtime/rollback.rs");
const CONSUMER_RUNTIME_SOURCE: &str = include_str!("../../domain/migration/runtime/consumer.rs");
...
fn stage9_stage10_consumer_census_adapter_only_narrows_the_production_closure_core() {
    ...
    assert!(
        CONSUMER_RUNTIME_SOURCE
            .contains("    pub(in crate::domain) fn evaluate_installation_snapshot(")
    );

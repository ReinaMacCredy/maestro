...
use crate::domain::vnext::installation::{
...
impl ActiveDomainInstallationClosureV1 {
    fn domain(&self) -> &crate::domain::vnext::distribution::runtime::DistributionDomainRefV1 {
        match self {
    ...
    fn release_id(&self) -> Option<crate::domain::vnext::distribution::ReleaseIdV1> {
        match self {
    ...
    fn to_store_object(
        &self,
    ) -> Result<StoreObjectV1, crate::domain::vnext::installation::InstallationClosureErrorV1> {
        match self {
...
pub struct ActivePublicationObjectsV1 {
    ...
    release_id: Option<crate::domain::vnext::distribution::ReleaseIdV1>,
...
fn validate_publication_objects(
    ...
    let expected_closure_kind = match transaction.plan().domain().kind() {
        crate::domain::vnext::distribution::runtime::DistributionDomainKindV1::RepositoryDomain => {
            DistributionRuntimeObjectKindV1::RepositoryInstallationClosure
        }
        crate::domain::vnext::distribution::runtime::DistributionDomainKindV1::InstallationDomain => {
    ...
    match (transaction.plan().mutation_kind(), &objects.migration) {
        (DistributionMutationKindV1::Migrate, Some((candidate, association))) => {
            let release_matches = match (
                ...
                (
                    Some(release_id),
                    crate::domain::vnext::migration::ReleaseBindingV1::InstallationExact(
                        migration_release_id,
                    ),
                ) => release_id.as_bytes() == migration_release_id.as_bytes(),
                (None, crate::domain::vnext::migration::ReleaseBindingV1::RepositoryAbsent) => true,
                _ => false,
            };
            if candidate.commit_record_ref().object_id() != commit.id()
                || candidate.association_object_id() != association.id()
                || association.schema_id()
                    != crate::domain::vnext::persistence::StoreCompatibilityV1::stage0_successor()?
...
pub enum InstallationOperationErrorV1 {
    ...
    #[error(transparent)]
    DistributionModel(
        #[from] crate::domain::vnext::distribution::runtime::DistributionModelErrorV1,
    ),
    #[error(transparent)]
    DistributionRecord(
        #[from] crate::domain::vnext::distribution::runtime::DistributionRecordErrorV1,
    ),
    #[error(transparent)]
    InstallationClosure(#[from] crate::domain::vnext::installation::InstallationClosureErrorV1),
    #[error(transparent)]

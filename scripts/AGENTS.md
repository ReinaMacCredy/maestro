...
| Task | Location | Notes |
|------|----------|-------|
| Build pipeline | `build.ts`, `build-lib.ts` | Injects build metadata; platform-specific cleanup |
...
| Skill syncing | `sync-bundled-skills.ts`, `check-agent-spec.ts` | Source is `skills/bundled/` |
...
- `build.ts` is stateful: it injects build metadata and handles platform-specific build cleanup.
- Skill drift is enforced by `bun run check:agent-spec` in CI/release paths. `build.ts` does not silently sync the bundled-skill template.

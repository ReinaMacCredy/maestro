...
Usage: bun scripts/lint/run-arch-lint.ts [--json] [--repo-root <path>] [--rules <path>]
...
async function main(): Promise<void> {
  ...
  if (args.json) {
    ...
    console.log(`architecture lint: clean (${report.filesScanned} file(s) scanned)`);
  } else {
    const counts = { error: 0, warn: 0, info: 0 };
    for (const v of report.violations) counts[v.severity]++;
    console.log(
      `architecture lint: ${report.violations.length} finding(s) across ${report.filesScanned} file(s) (${counts.error} error, ${counts.warn} warn, ${counts.info} info)`,
    );

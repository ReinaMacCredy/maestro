 ...
 * Hot-Path Smoke Test
 *
 * Exercises the full task lifecycle against a freshly installed maestro
 * binary. Intended for use in CI after install and before release publish.
 *
 * Usage:
 *   bun scripts/smoke.ts                     # uses maestro on PATH
 *   bun scripts/smoke.ts ./dist/maestro       # uses a specific binary
 *
 * Exits non-zero on any hot-path verb failure.
 *
...
async function main(): Promise<void> {
  tmpDir = await mkdtemp(join(tmpdir(), "maestro-smoke-"));
  ...
  const specOut = await run(["spec", "new", "smoke-spec", "--title", "Smoke Test"], tmpDir);
  console.log(specOut);
  ...
  console.log("\n[smoke] PASS: all hot-path verbs succeeded");
}

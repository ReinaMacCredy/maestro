...
describe("UKI handoff roundtrip", () => {
  ...
  it("create execute -> list -> pickup default UKI -> parseUki round-trips", async () => {
    ...
    expect(created.content.mode).toBe("execute");
      ...
      const listed = await runCli(["handoff", "list", "--json"], tmpDir);
    expect(listed.exitCode).toBe(0);
    expect(JSON.parse(listed.stdout)).toHaveLength(1);

      const pickup = await runCli(["handoff", "pickup", "--id", created.id], tmpDir);
  ...
  it("create --paste returns an agent-ready preamble plus the raw UKI", async () => {
    ...
    const [preamble, uki] = create.stdout.split("\n\n");
      expect(preamble).toBe(HANDOFF_PASTE_PREAMBLE);
  ...
  it("pickup --paste returns an agent-ready preamble plus the raw UKI", async () => {
    ...
    const { id } = JSON.parse(create.stdout);
      ...
      const pickup = await runCli(["handoff", "pickup", "--id", id, "--paste"], tmpDir);
    expect(pickup.exitCode).toBe(0);
  ...
  it("pickup canonicalizes legacy v5.3 records to v5.4 UKI output", async () => {
    ...
    await Bun.write(
      ...
      const pickup = await runCli(["handoff", "pickup", "--id", "2026-04-09-123"], tmpDir);
    expect(pickup.exitCode).toBe(0);
    expect(pickup.stdout.startsWith("MODE-execute|")).toBe(true);
  ...
  it("errors cleanly when required mode is missing or no pending handoff exists", async () => {
    ...
    expect((missingMode.stdout + missingMode.stderr).toLowerCase()).toContain("mode");
      ...
      const noPending = await runCli(["handoff", "pickup"], tmpDir);

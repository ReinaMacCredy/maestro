...
import { JsonlExecPlanStore } from "@/v2/repo/jsonl-exec-plan-store.adapter.js";
import {
...
function makeStore(root: string): JsonlExecPlanStore {
  let n = 0;
  return new JsonlExecPlanStore({
    repoRoot: root,
    clock: () => FROZEN,
    idFactory: () => `pln-${++n}`,
  });
...
describe("JsonlExecPlanStore", () => {
  ...
  it("throws ExecPlanNotFoundError on update of missing id", async () => {
    const store = makeStore(root);
  ...
  it("persists each plan as one JSONL line", async () => {
    ...
    const text = await readFile(join(root, ".maestro/plans/plans.v2.jsonl"), "utf8");

...
import type { ExecuteUkiHandoffContent, PlanUkiHandoffContent } from "../../../src/domain/uki-types.js";
import { compressUki, parseUki, validateUki } from "../../../src/lib/uki-format.js";

const PLAN_FIXTURE: PlanUkiHandoffContent = {
  mode: "plan",
  ...
  planPaths: ["plan_md", "maestro_reference_handoff_redesign_plan_md"],
  ...
  readMore: ["plan_md", "maestro_reference_handoff_redesign_plan_md"],
...
const EXECUTE_FIXTURE: ExecuteUkiHandoffContent = {
  mode: "execute",
...
describe("UKI v5.4 format", () => {
  it("compresses a plan payload in deterministic order", () => {
      expect(compressUki(PLAN_FIXTURE)).toBe(
        "MODE-plan"
        ...
        + "|PLAN_PATHS-plan_md-maestro_reference_handoff_redesign_plan_md"
        + "|MAESTRO_SYNC-mission_created-mission_approved"
      + "|DECISIONS-structured_json_source_of_truth-pickup_defaults_to_uki"
      + "|SIGNAL_DELTA-mission_0_1-milestones_0_2"
      + "|ARTIFACTS-mission_2026_04_09_001-file_plan_md"
      + "|READ_MORE-plan_md-maestro_reference_handoff_redesign_plan_md"
  ...
  it("compresses an execute payload with optional blind spot and metaphor", () => {
      expect(compressUki(EXECUTE_FIXTURE)).toBe(
        "MODE-execute"
  ...
  it("round-trips plan and execute payloads", () => {
    expect(parseUki(compressUki(PLAN_FIXTURE))).toEqual(PLAN_FIXTURE);
    expect(parseUki(compressUki(EXECUTE_FIXTURE))).toEqual(EXECUTE_FIXTURE);
  });

  it("accepts six-word underscore tokens in v5.4 output", () => {
    const compressed = compressUki(EXECUTE_FIXTURE);
  ...
  it("flags forbidden characters and overlong summary values", () => {
    const withColon = compressUki(PLAN_FIXTURE).replace(
      "SESSION_CORE-handoff_rebuild_plan",
    ...
    const withLongSummary = compressUki(PLAN_FIXTURE).replace(
      "SUMMARY-Handoff_plan_saved-mission_setup_complete-ready_to_execute",
      longSummary,
    );

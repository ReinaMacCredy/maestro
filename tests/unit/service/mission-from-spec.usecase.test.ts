...
function makeStores(): {
  ...
  return { missionStore, plans, evidenceStore, evidence };
}
...
describe("missionFromSpec", () => {
  ...
  it("creates an Mission in 'specified' from a heavy-mode spec and emits one transition row", async () => {
    ...
    const plan = await missionFromSpec(
      { repoRoot, missionStore, evidenceStore },
  ...
  it("throws MissionRequiresHeavyModeError for a light-mode spec", async () => {
    const specPath = join(repoRoot, ".maestro/specs/demo-light.md");
    await writeFile(specPath, LIGHT_SPEC);
    const { missionStore, evidenceStore, evidence } = makeStores();

    await expect(
      missionFromSpec({ repoRoot, missionStore, evidenceStore }, ".maestro/specs/demo-light.md"),
    ).rejects.toBeInstanceOf(MissionRequiresHeavyModeError);
  ...
  it("rejects duplicate slug via the underlying store", async () => {
    const specPath = join(repoRoot, ".maestro/specs/demo-plan.md");
    await writeFile(specPath, HEAVY_SPEC);
    const { missionStore, evidenceStore } = makeStores();

    await missionFromSpec({ repoRoot, missionStore, evidenceStore }, ".maestro/specs/demo-plan.md");
    await expect(
      missionFromSpec({ repoRoot, missionStore, evidenceStore }, ".maestro/specs/demo-plan.md"),

...
1. "Migrate to v2 first: run `maestro setup`."
2. "Author a heavy spec for a reporting module: weekly digest, PDF export, and
   email delivery."
3. "Run `mission from-spec` then `mission decompose` -- break into at least 3 tasks."
...
5. At least 2 child task rows with `to_state: "draft"` and `mission_id` set.

...
describe("maestro plan decompose (v2)", () => {
  it("creates child tasks linked by plan_id and advances the plan to 'planned'", async () => {
    ...
    for (const t of tasks) {
      expect(t.plan_id).toBe(planId);
    }
    const slugs = tasks.map((t) => t.slug).sort();
    expect(slugs).toEqual(["first", "second", "third"]);

    const rows = await readEvidenceRows(tmpDir);
    const planFromSpec = rows.find(
      (r) => r.trigger_verb === "plan:from-spec",
    );
    const planDecompose = rows.find(
      (r) => r.trigger_verb === "plan:decompose",
    );
    const taskRows = rows.filter((r) => r.trigger_verb === "task:from-spec");
    expect(planFromSpec).toBeDefined();
    expect(planDecompose).toMatchObject({
      kind: "transition",
      plan_id: planId,
  ...
  it("fails when the plan is already planned (transition rejected)", async () => {
    ...
    const second = await runCompiled(
      ["plan", "decompose", planId, "--file", batch2Path],
      tmpDir,
    );
    expect(second.exitCode).toBe(1);
    expect(second.stderr).toContain("Invalid exec-plan transition planned -> planned");
  });
  ...
  it("fails for a missing plan id", async () => {
    ...
    await writeFile(batchPath, JSON.stringify([{ title: "A", slug: "a" }]));
    const result = await runCompiled(
      ["plan", "decompose", "pln-doesnotexist", "--file", batchPath],
      tmpDir,
    );
    expect(result.exitCode).toBe(1);

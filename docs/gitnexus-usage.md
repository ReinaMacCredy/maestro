...
This project is indexed by GitNexus as **maestro** (10765 symbols, 18180
...
- **Run impact analysis before editing any symbol.** Before modifying a
  ...
  direction: "upstream"})` and report the blast radius (direct callers,
  affected processes, risk level) to the user.
- **Run `gitnexus_detect_changes()` before committing** to verify your
...
- When you need full context on a specific symbol — callers, callees, which
  execution flows it participates in — use `gitnexus_context({name:
  "symbolName"})`.
